...
export class F1DataService {
  ...
  public async getTeamRadio(sessionKey: string, driverNumber: string): Promise<TeamRadioData[]> {
    try {
  ...
  public async getRaceControlMessages(sessionKey: string): Promise<RaceControlData[]> {
    try {
  ...
  public async getDriverComparison(sessionKey: string, driver1: string, driver2: string, lap: number): Promise<TelemetryData> {
    try {
  ...
  async getLapAnalysis(
    ...
    const lapData = lapTiming.find(lt => 
      lt.driver_number === driverNumber && 
  ...
  public async getQualifyingAnalysis(sessionKey: string): Promise<QualifyingData> {
    try {
  ...
  async getDriverCareerStats(driverId: string): Promise<DriverCareerData> {
    ...
    const championships = (championshipResults?.MRData?.StandingsTable?.StandingsLists || []) as ChampionshipStanding[];
    ...
    const championshipWins = championships.filter(standing => 
  ...
  async getSeasonComparison(
    ...
    const [season1, season2] = await Promise.all([
      this.fetchWithErrorHandling<SeasonData>(
        `${FASTF1_BASE_URL}/${year1}/results.json${driverId ? `?driver=${driverId}` : ''}`,
        `Failed to fetch ${year1} season data`
      ),
      this.fetchWithErrorHandling<SeasonData>(
        `${FASTF1_BASE_URL}/${year2}/results.json${driverId ? `?driver=${driverId}` : ''}`,
  ...
  public async getSprintResults(year: number, round: number): Promise<SprintData> {
    try {
